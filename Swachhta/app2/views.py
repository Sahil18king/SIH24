...
from .ml_model import model, calculate_cleanliness_and_dirtiness, generate_graph
...
def capture_image(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            img_path = default_storage.path(image_path)
            img = cv2.imread(img_path)
            img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

            # Calculate cleanliness and dirtiness
            average_cleanliness, average_dirtiness = calculate_cleanliness_and_dirtiness(model, img_rgb)

            # Generate the graph
            fig, ax = plt.subplots()
            ax.bar(['Cleanliness', 'Dirtiness'], [average_cleanliness, average_dirtiness])
            ax.set_title('Cleanliness vs Dirtiness')
